...
const SellerDashboard = () => {
  ...
  const {user} = useUser();
  const stripeConnectId = useQuery(api.queries.users.getUsersStripeConnectId, {
    userId: user?.id || "",
  });
  ...
  const isReadytoTakePayments =
    accountStatus?.isActive && accountStatus.payoutsEnabled;
const hanldeManageAccount =async()=>{
  try {
    if (stripeConnectId && accountStatus?.isActive) {
      const loginUrl = await stripeConnectLoginLink(stripeConnectId);
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="bg-gray-50 rounded-lg p-4">
                  ...
                  <div className="mt-2 flex items-center">
                    <div
                      className={`size-3 rounded-full mr-2 ${accountStatus.isActive ? "bg-green-500" : "bg-yellow-500"}`}
                    >
                      <span className="text-lg font-semibold">
                        {accountStatus?.isActive ? "Active" : "Pending Status"}
                      </span>
                    </div>
                  </div>
                  ...
                  <div className="bg-gray-50 rounded-lg p-4">
                    ...
                    <div className="mt-2 space-y-1">
                      <div className="flex items-center">
                        <svg
                          className={`size-5 ${accountStatus.chargesEnabled ? "text-green-500" : "text-gray-500"}`}
                          fill="currentColor"
                          viewBox="0 0 20 20"
                        >
                          <path
                            fillRule="evenodd"
                            d="M10 18a8 8 0 10016 8 8 0 000 16zm3.707-9.293a1 1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z"
                            clipRule="evenodd"
                          />
                        </svg>
                        <span className="ml-2">
